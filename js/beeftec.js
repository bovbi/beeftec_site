(function($) {
  ...
  var script = '<script type="text/javascript" src="http://www.noticiasagricolas.com.br/widget/cotacoes.js.php?id=160&fonte=Arial%2C%20Helvetica%2C%20sans-serif&tamanho=10pt&largura={{widthNoticias}}px&cortexto=333333&corcabecalho=FFFFFF&corlinha=ede7bf&imagem=true&output=js"></script>'.replace(
    "{{widthNoticias}}",
    widthNoticias
  );
  ...
  $("#formulario-contato").submit(function(event) {
    ...
    $.ajax({
      url: "https://api.beeftec.com.br/v1/site/send_contact_email",
      data:
        $("#formulario-contato").serialize() + "&ts=" + new Date().getTime(),
      method: "POST"
    })
      .done(function(data) {
        $("#messageSuccess").show();
